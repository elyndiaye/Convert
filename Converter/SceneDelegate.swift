...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func updateTabBar() -> UITabBarController {
           ...
           tabbar.viewControllers = [createConverterVC(),createConverterListVC()]
           return tabbar
       }
    
    func createConverterListVC() -> UINavigationController {
        let converterVC = ConverterListViewController(isSelectedTable: false)
        let navController = UINavigationController(rootViewController: converterVC)
        converterVC.title = "List"
        converterVC.tabBarItem = UITabBarItem(
            title: "List",
            image: UIImage.init(named: "list_icon")?.withRenderingMode(.alwaysTemplate), tag: 1)
        return navController
    }
    
    func createConverterVC() -> UINavigationController {
